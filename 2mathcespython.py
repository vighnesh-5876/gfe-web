...
def get_team_data():
    teams = []
    for i in range(1, 4):  # Updated to handle 3 teams
        team_name = input(f"Enter the name of Team #{i}: ")
        placement_match1 = int(input(f"Enter the placement for {team_name} in Match 1 (1-3): "))
        kills_match1 = int(input(f"Enter the kill points for {team_name} in Match 1: "))
        placement_match2 = int(input(f"Enter the placement for {team_name} in Match 2 (1-3): "))
...
def main():
    ...
    export_to_excel(point_table, '2matchexcel.xlsx')
